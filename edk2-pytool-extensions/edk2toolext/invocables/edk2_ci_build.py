    ...
    def Go(self):
        ...
        for pkgToRunOn in self.requested_package_list:
            ...
            _, txt_handle = edk2_logging.setup_txt_logger(
                packagebuildlog_path, f"BUILDLOG_{os.path.basename(pkgToRunOn)}", logging_level=logging.DEBUG, isVerbose=True)
            ...
            pkg_config_file = edk2path.GetAbsolutePathOnThisSytemFromEdk2RelativePath(
                os.path.join(pkgToRunOn, os.path.basename(pkgToRunOn) + ".ci.yaml"))
            if(pkg_config_file):
